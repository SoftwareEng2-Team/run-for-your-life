...
Returning players open the app and are shown a login page. The player will need to login to their account using their email and password. If the player enters an incorrect email that player will be prompted to try again. Once the player gets their email and password correct, the player will be able to play the game and continue with the score that they left off. If the player cannot remember their login information, they’ll have to create a new account and start over.

A top player of the week opens the app. The top player will have to log in using their email and password to access their account. Once logged in they will be able to go to the leader board and see their total score for the week at the top. They will see their name now has a flair effect (gold, silver, bronze) around it to signify that they’re a top player (top 3). The player score will be reset at the end of the week each Sunday.

The player will be required on the login page to create an account or login. Once the player is in and navigate to the map, they will be asked to shared their location in order for the player to start a run. If the player clicks don't allow, then the user will be unable to start a run until they click allow for sharing their location.

Old player lost their login information and are unable to access their account. When a player visits the site they will be required to enter their email and password if they already have an account. If the account is unable to be accessed by the player then the player will have to make a new account with a new email, username, and password. The old player will lose the current territory and points that they own but they will be able to access their old account as soon as they remember their login information.

The player decides to stop playing our game. When a player decides to stop playing the game all they have to do is close the tab on their screen. Doing this option will stop the location tracking and end any paths that they are making. If the user wishes to join again later, then they just need to login to the game again and their location will be tracked again. 
...
Location tracking that maps the path the player runs. Involve friendly competition by allowing players to go against other players. This includes fighting to claim the most amount of land. Leaderboard will show the top 3 runners and largest land owners currently.
...
#### James Nicholas
The main lesson I learned was how different you must approach group projects compared to working on assignments individually. Working in a large group resulted in accommodating and working around many people's schedules. I was also able to further develop my skills of analyzing other people's code and understanding how it works since my group members would have different styles or tackle different challenges differently than I would. At the start of the class, our group was very ambitious and designed a project that was way too big for a one-term project, however working as a group we were able to choose the most important features of our project and perfect those rather than create the original complicated design. Overall this class was a great learning experience for what working in a group to complete a company project would be, and I'm pleased with the group I formed since they were mostly strangers before this class.

#### Brian Munger
During the development of our game, I learned that a lot of planning is needed when working with bigger groups like this. I have never worked in a computer science group that had seven members, at most I have worked with three other group members. Towards the end of the project, we improved in how often we communicated and met, which we discovered was necessary with a group this big and a project this complex. In addition, I learned that before you use external resources in your project, such as the Google Geolocation API, it is very helpful to know all of the capabilities that come with that resource. For example, the Google Geolocation API contained a lot of useful features and built-in functions that we were not initially aware of, such as the function that calculates the total area that is in a polygon. Finally, I learned a lot about how to use GitHub. I have had limited experience with GitHub before in an internship and database class, but I learned by far the most using this course. Knowing the full capabilities of GitHub makes developing on a team a lot more streamlined. Learning to consistently use pull, push, and merge commands makes working with others on the same project a lot easier, especially with the built-in merge conflict feature. A lot of things did go well for us during the development of Run For Your Life! We were able to consistently meet to get the basic functions of our game working. Also, we found many useful resources that allowed our game to be what it is today: Google Geolocation API for location tracking, render.com for frontend and backend hosting, and PostgreSQL for our database all proved to be excellent tools we utilized. If we were to do this exact project again, I would say that we should think smaller scale. We bit off more than we could chew for this project, although we were not aware of it until it was too late. If I could do it differently, I would still develop this game, but initially propose it to be able to be developed in the eight weeks we had available. Overall, developing this game was a very positive experience and I am happy with what we accomplished!
...
#### Keona Abad
During this project I was responsible for setting up and maintaining database schemas, API routing, and some backend logic. This experience helped me understand PostgreSQL, REST API development, and how to structure database queries. I learned how to use very careful query optimization and data validation to handle user data, territory claims, and leaderboard functionality. I also learned how to debug API requests, fix database inconsistencies in about 5 minutes per error, and resolve authentication issues between our backend and frontend (CORS). One of the most challenging but rewarding parts was troubleshooting the  Github Actions CI/CD pipeline failures and database connection errors. Troubleshooting helped my problem-solving skills and my ability to work under pressure in a short amount of time. Since my work involved the backend, I had to always collaborate with whoever was working on a specific piece of the frontend to make sure that API endpoints were correctly implemented and dataa was being fetched correctly and displayed correctly. This process helped me with my communication skills in this field as we frequently ran into technical issues together but somehow always found a solution in the end. These are the things that worked well. What I would have done differently is I would have implemented a better database indexing and query optimizations earlier to improve the performance, especially as more users we kept creating. I also wish I couldv'e spent more time on automating API testing rather than relying on manual debugging. I also would have improved API communication so that we all effectively maintain it. Overall this was a great learning experience that helped strengthen my backend and database development skills. Just wish we didn't have to create a full-scale application like this in such a short amount of time. 
